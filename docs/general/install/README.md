...
```hcl
module "amplify" {
    source = "github.com/pbs/terraform-amplify-module?ref=x.y.z"
}
```
...
gh -R 'pbs/terraform-amplify-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/amplify'
rm -f release.tar.gz
```
...
```bash
git remote add -f amplify git@github.com:pbs/terraform-amplify-module.git
git subtree add --prefix terraform/modules/amplify amplify main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/amplify amplify $REF --squash
```
